...
export default async function AllTests({ teacherId }: Props) {
  ...
  const testIds = tests.map((t: any) => t._id);
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      {tests.map((t: any) => {
        const attempts = countsMap[String(t._id)] ?? 0;
          ...
          >
            ...
            <div className="p-5 flex-1">
              ...
              <div className="mt-3 flex items-center flex-wrap gap-3 text-xs text-slate-500">
                ...
                {/* attempts badge */}
                <span className="ml-2 px-2 py-0.5 rounded-full bg-slate-100 text-slate-700 text-xs font-medium">
